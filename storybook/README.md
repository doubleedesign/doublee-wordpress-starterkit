...
My parent themes are included in this repository as submodules, i.e., they are their own repositories as well, so they need to be pulled in when setting this project up if you're going to use them. To pull in _just_ the theme(s) and not the plugins, run:

```bash
git submodule update --init app/public/wp-content/themes/starter-kit-classic
```
```bash
git submodule update --init app/public/wp-content/themes/starter-kit-blocks
```
...
I am writing this in the context of working with my starter kits as parent themes, with child themes for individual projects. Both can contain PHP templates for WordPress blocks, and the child theme contains other elements such as the site header and footer templates.

So that Storybook can access these files in a useful and intuitive structure without duplicating files, I use symlinking. For example: 

| Real directory                                                | Symlinked directory                             |  
|---------------------------------------------------------------|-------------------------------------------------|
| `app/public/wp-content/themes/starter-kit-classic/components` | `storybook/components/classic-theme/components` |
| `app/public/wp-content/themes/starter-kit-classic/modules`    | `storybook/components/classic-theme/modules`    |
| `app/public/wp-content/themes/starter-kit-blocks/components`  | `storybook/components/block-theme/components`   |
| `app/public/wp-content/themes/starter-kit-blocks/blocks`      | `storybook/components/block-theme/blocks`       |

**Note:** to symlink directories in Windows, you need to do it natively - not in WSL. A PowerShell script for creating the above symlinks for the parent themes is located in `./symlinks.ps1`.
...
Check that this worked by navigating to `http://localhost:6001` in your browser. You should see "Hello PHP!"
